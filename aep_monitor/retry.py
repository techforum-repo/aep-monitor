...
"""Retry policy for transient Adobe API failures.
...
backoff; permission denied (401/403) and missing configuration are not.

A 429 specifically carries Adobe's own `Retry-After` hint (see
`errors.AdobeRateLimitError`) when Adobe sent one — honored in place of the
exponential guess. `clients.base.RequestPacer` handles the *proactive* side
(pacing requests so 429s are rare in the first place); this is the reactive
fallback for when one happens anyway.
"""
...
def call_with_retry(
    ...
    *,
    ...
    is False (permission denied, missing configuration, ...).
    """
